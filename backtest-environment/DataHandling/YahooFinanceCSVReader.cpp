...
YahooFinanceCSVReader:: YahooFinanceCSVReader(char *symbol, long startdate, long enddate, char outfilename[FILENAME_MAX], char cookiefilename[FILENAME_MAX], char crumbfilename[FILENAME_MAX]) : marketmovements() {
    ...
    string down_url = string("https://query1.finance.yahoo.com/v7/finance/download/") + string(symbol) + string("?period1=") + to_string(startdate) + string("&period2=") + to_string(enddate) + string("&interval=") + string(interval) + string("&events=history&crumb=") + string(get_crumb_and_cookies(symbol, cookiefilename, crumbfilename));
