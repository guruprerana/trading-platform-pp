...
std::map<long, double> Strategy::get_data(int N, int k){
    ...
    std::map<long, double> price_map = mapData[this->get_price_type()]; //take the map <long, double> associated to the price_type chosen at the beginning
...
std::tuple<double, double> Strategy::auxiliary_linear_regression(std::map<long, double> &bars){
    assert(!bars.empty());
...
void Strategy::update_numericSignals(){
